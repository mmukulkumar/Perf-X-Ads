...
import { Search, Sparkles, Bot, TrendingUp, AlertCircle, RefreshCw, BarChart2, MessageSquare, BookOpen, Layers, ArrowLeft, Globe, Zap, LineChart, Laptop, FlaskConical } from 'lucide-react';
...
const AiKeywordResearch = () => {
  ...
  const handleAnalyze = async (type: 'keyword' | 'brand', inputValue: string) => {
    if (!inputValue.trim()) {
    ...
    try {
        ...
        const contextInstruction = type === 'brand' 
                ...
                <div className="flex-1 bg-[#FFF5F0] dark:bg-[#2C1810] rounded-3xl p-8 relative overflow-hidden group border border-[#FFE0D0] dark:border-[#4A2818] transition-all hover:shadow-lg">
                        ...
                        <div className="mt-auto">
                            <div className="bg-white dark:bg-gray-800 p-2 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col sm:flex-row gap-2">
                                <input 
                                    ...
                                    onKeyDown={(e) => {
                                        if (e.key === 'Enter') handleAnalyze('keyword', e.currentTarget.value);
                                    }}
                                />
                                <button 
                                    onClick={(e) => {
                    ...
                    <div className="hidden lg:block absolute bottom-24 right-8 pointer-events-none opacity-90">
                        <div className="relative w-32 h-32">
                             ...
                             <svg viewBox="0 0 100 100" className="w-full h-full text-gray-800 dark:text-gray-200 fill-current">
                        ...
                        <div className="mt-auto">
                            <div className="bg-white dark:bg-gray-800 p-2 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col sm:flex-row gap-2">
                                <input 
                                    ...
                                    className="flex-1 px-4 py-3 bg-transparent outline-none text-gray-800 dark:text-gray-100 placeholder-gray-400"
                                    onKeyDown={(e) => {
                                        if (e.key === 'Enter') handleAnalyze('brand', e.currentTarget.value);
                                    }}
                                />
                                <button 
                                    onClick={(e) => {
                ...
                <div className="flex items-center justify-between mb-2">
                    ...
                    {isAnalyzing && (
                        <div className="flex items-center gap-2 text-indigo-600 dark:text-indigo-400 font-bold text-sm animate-pulse">
                            <RefreshCw className="w-4 h-4 animate-spin" /> Analyzing {searchType}...
                        </div>
                    )}
                </div>

                {error && <div className="p-4 bg-red-50 text-red-600 rounded-xl border border-red-100 flex items-center gap-2"><AlertCircle className="w-5 h-5" /> {error}</div>}

                {/* 1. Summary */}
                {result && (
                    <div className="bg-brand-surface p-6 rounded-xl border border-brand-medium/30 shadow-sm relative overflow-hidden">
                        <div className={`absolute top-0 left-0 w-1 h-full ${searchType === 'brand' ? 'bg-indigo-500' : 'bg-orange-500'}`}></div>
                        <h3 className="text-lg font-bold text-brand-dark mb-3 flex items-center gap-2">
                            <BarChart2 className={`w-5 h-5 ${searchType === 'brand' ? 'text-indigo-500' : 'text-orange-500'}`} /> 
                            Analytical Summary: <span className="text-brand-dark/60 ml-2 font-normal">"{searchInput}"</span>
                        </h3>
                        <p className="text-brand-dark/80 leading-relaxed">{result.summary}</p>
                    </div>
                )}
                                ...
                                <tbody className="divide-y divide-brand-medium/10">
                                    {result.keywords.map((k, idx) => (
                                        <tr key={idx} className="hover:bg-brand-light/30 transition-colors">
                                            <td className="p-4 font-bold text-brand-dark">{k.keyword}</td>
                                            <td className="p-4 text-sm text-brand-dark/70">
                                                <span className="bg-brand-medium/10 px-2 py-1 rounded text-xs">{k.intent}</span>
                                            </td>
                                            <td className="p-4 text-sm font-mono text-brand-dark/80">{k.traffic}</td>
                                            <td className="p-4 text-sm">
                                                <span className={`px-2 py-1 rounded text-xs font-bold ${
                                                    k.difficulty.toLowerCase() === 'high' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300' :
                                                    k.difficulty.toLowerCase() === 'medium' ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-300' :
                                                    'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300'
                                                }`}>{k.difficulty}</span>
                                            </td>
                                            <td className="p-4">
                                                <div className={`inline-flex items-center justify-center w-8 h-8 rounded-full text-sm font-bold border ${getVisibilityColor(k.llmVisibility)}`}>
                                                    {k.llmVisibility}
                                                </div>
                                            </td>
                                            <td className="p-4 text-sm text-brand-dark/80 leading-snug">{k.strategy}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                ...
                {result && (
                    <div className="bg-[#FF4500]/5 dark:bg-[#FF4500]/10 rounded-xl border border-[#FF4500]/20 p-6">
                        <h3 className="text-lg font-bold text-[#FF4500] mb-4 flex items-center gap-2">
                            <MessageSquare className="w-5 h-5" /> Community & User Intent Insights (Reddit)
                        </h3>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="p-4 bg-brand-surface rounded-lg border border-brand-medium/10 shadow-sm">
                                <h4 className="text-xs font-bold text-brand-dark/50 uppercase mb-2">Primary Pain Point</h4>
                                <p className="text-sm text-brand-dark font-medium">{result.redditInsights.painPoint}</p>
                            </div>
                            <div className="p-4 bg-brand-surface rounded-lg border border-brand-medium/10 shadow-sm">
                                <h4 className="text-xs font-bold text-brand-dark/50 uppercase mb-2">Content Format Suggestion</h4>
                                <p className="text-sm text-brand-dark font-medium">{result.redditInsights.formatSuggestion}</p>
                            </div>
                            <div className="p-4 bg-brand-surface rounded-lg border border-brand-medium/10 shadow-sm">
                                <h4 className="text-xs font-bold text-brand-dark/50 uppercase mb-2">Language/Slang Alert</h4>
                                <p className="text-sm text-brand-dark font-medium">{result.redditInsights.slangAlert}</p>
                            </div>
                        </div>
                    </div>
                )}
                ...
                {result && (
                    <div className="bg-brand-surface p-6 rounded-xl border border-brand-medium/30 shadow-sm">
                        <h3 className="text-sm font-bold text-brand-dark mb-4 flex items-center gap-2">
                            <BookOpen className="w-4 h-4 text-brand-medium" /> Metric Definitions
                        </h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 text-xs text-brand-dark/70">
                            <div>
                                <span className="font-bold text-brand-dark block mb-1">Monthly Traffic (Est.)</span>
                                {result.definitions.traffic || "A quantitative estimate of monthly search volume."}
                            </div>
                            <div>
                                <span className="font-bold text-brand-dark block mb-1">Traditional Rank Difficulty</span>
                                {result.definitions.difficulty || "Qualitative assessment of competition in classic search."}
                            </div>
                            <div>
                                <span className="font-bold text-brand-dark block mb-1">LLM Visibility Probability</span>
                                {result.definitions.visibility || "Likelihood of content being cited in AI Overviews or used by models like ChatGPT."}
                            </div>
                            <div>
                                <span className="font-bold text-brand-dark block mb-1">Ranking Strategy</span>
                                {result.definitions.strategy || "Actionable suggestion for content creation tailored to RAG data."}
                            </div>
                        </div>
                    </div>
                )}
